...
const CandlestickChart = () => {
  ...
  const candles = useMemo(() => {
    ...
    trades.forEach((trade) => {
      ...
      if (!grouped[intervalStart]) {
        ...
        grouped[intervalStart].high = Math.max(grouped[intervalStart].high, price);
        grouped[intervalStart].low = Math.min(grouped[intervalStart].low, price);
  ...
  const candlesWithMA = useMemo(() => {
    ...
    return candles.map((candle, index) => {
      const ma7 = index >= 6 
        ? candles.slice(index - 6, index + 1).reduce((sum, c) => sum + c.close, 0) / 7
        : 0;
      
      const ma25 = index >= 24
        ? candles.slice(index - 24, index + 1).reduce((sum, c) => sum + c.close, 0) / 25
        : 0;
      
      const ma99 = index >= 98
        ? candles.slice(index - 98, index + 1).reduce((sum, c) => sum + c.close, 0) / 99
        : 0;
  ...
  const candleInfo = useMemo(() => {
    ...
    const changePercent = selectedCandle.open !== 0 
      ? (change / selectedCandle.open) * 100 
      : 0;
    const amplitude = selectedCandle.open !== 0
      ? ((selectedCandle.high - selectedCandle.low) / selectedCandle.open) * 100
      : 0;
  ...
  useEffect(() => {
    ...
    const drawChart = () => {
      ...
      const spacing = candlesWithMA.length > 1 
        ? availableWidth / (candlesWithMA.length - 1) 
        : 0;
      const candleWidth = Math.min(8, Math.max(2, spacing > 0 ? spacing * 0.4 : 6));
      ...
      const volumeToY = (volume: number) => {
        ...
        return chartHeight + volumeHeight - volumePadding - ratio * (volumeHeight - 2 * volumePadding);
      };
      ...
      candlesWithMA.forEach((candle, index) => {
        ...
        const volumeBarHeight = chartHeight + volumeHeight - volumePadding - volumeBarTop;
      ...
      if (mousePosition && mousePosition.x >= padding && mousePosition.x <= width - rightPadding) {
      ...
      if (candlesWithMA.length > 0) {
        ...
        const volUSDT = ((lastCandle.volume * lastCandle.close) / 1e11).toFixed(2);
        ctx.fillStyle = "#848E9C";
        ctx.font = "11px 'JetBrains Mono', monospace";
        ctx.textAlign = "left";
        ctx.fillText(
  ...
  }, [candlesWithMA, maxPrice, minPrice, maxVolume, hoveredCandle, mousePosition]);
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (x >= leftPadding && x <= cssWidth - rightPadding && candlesWithMA.length > 0 && spacing > 0) {
  ...
  return (
    <div className="candlestick-chart">
        ...
        {candleInfo && (
          <div className="chart-info">
            <div className="chart-info-row">
              <span className="info-date">{formatDate(candleInfo.timestamp)}</span>
              ...
              <span className="info-value">{candleInfo.amplitude.toFixed(2)}%</span>
            </div>
            <div className="chart-info-row">
              <span className="info-label">MA(7):</span>
              <span className="info-value ma-7">{candleInfo.ma7 > 0 ? candleInfo.ma7.toFixed(2) : "0.00"}</span>
              <span className="info-label">MA(25):</span>
              <span className="info-value ma-25">{candleInfo.ma25 > 0 ? candleInfo.ma25.toFixed(2) : "0.00"}</span>
              <span className="info-label">MA(99):</span>
              <span className="info-value ma-99">{candleInfo.ma99 > 0 ? candleInfo.ma99.toFixed(2) : "0.00"}</span>
            </div>
          </div>
        )}
      </div>
      ...
      {mousePosition && hoveredCandle !== null && candlesWithMA[hoveredCandle] && (
        <div
